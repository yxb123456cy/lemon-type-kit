...
describe('number utils', () => {
  describe('toNumber', () => {
    ...
    it('should convert number to number', () => {
      ...
      expect(numberUtils.toNumber(-1.5)).toBe(-1.5);
    });

    // 应该将字符串转换为数字
    it('should convert string to number', () => {
      expect(numberUtils.toNumber('123')).toBe(123);
      expect(numberUtils.toNumber('12.3')).toBe(12.3);
      expect(numberUtils.toNumber('  456  ')).toBe(456);
    });

    // 应该对无效输入返回 NaN
    it('should return NaN for invalid inputs', () => {
      expect(numberUtils.toNumber('abc')).toBeNaN();
      expect(numberUtils.toNumber('')).toBeNaN();
      expect(numberUtils.toNumber('  ')).toBeNaN();
      expect(numberUtils.toNumber(undefined)).toBeNaN();
      expect(numberUtils.toNumber({})).toBeNaN();
    });
  ...
  describe('clamp', () => {
    ...
    it('should clamp value within range', () => {
      ...
      expect(numberUtils.clamp(-5, 0, 5)).toBe(0);
      expect(numberUtils.clamp(3, 0, 5)).toBe(3);
    });

    // 应该处理浮点数
    it('should handle floating point numbers', () => {
      expect(numberUtils.clamp(10.5, 0, 5)).toBe(5);
      expect(numberUtils.clamp(2.5, 0, 5)).toBe(2.5);
    });

    // 如果 min > max 应该抛出错误
    it('should throw error if min > max', () => {
      expect(() => numberUtils.clamp(1, 5, 0)).toThrow();
    });
  });
  ...
  describe('random', () => {
    ...
    it('should handle min equals max', () => {
      expect(numberUtils.random(5, 5)).toBe(5);
    });

    // 如果 min > max 应该抛出错误
    it('should throw error if min > max', () => {
      expect(() => numberUtils.random(10, 5)).toThrow();
    });
  });
  ...
  describe('isNegative', () => {
    ...
    it('should return true for negative numbers', () => {
      ...
      expect(numberUtils.isNegative(-0.5)).toBe(true);
    });
  ...
  describe('sum', () => {
    ...
    it('should calculate sum of array', () => {
      ...
      expect(numberUtils.sum([-1, 1])).toBe(0);
  ...
  describe('average', () => {
    ...
    it('should calculate average of array', () => {
      ...
      expect(numberUtils.average([2, 4])).toBe(3);
    });
  ...
  describe('min', () => {
    ...
    it('should return minimum value', () => {
      ...
      expect(numberUtils.min([3, 2, 1])).toBe(1);
    });

    // 空数组应该返回 Infinity
    it('should return Infinity for empty array', () => {
      expect(numberUtils.min([])).toBe(Infinity);
    });
  });
  ...
  describe('max', () => {
    ...
    it('should return maximum value', () => {
      ...
      expect(numberUtils.max([3, 2, 1])).toBe(3);
    });

    // 空数组应该返回 -Infinity
    it('should return -Infinity for empty array', () => {
      expect(numberUtils.max([])).toBe(-Infinity);
    });
  });
  ...
  describe('inRange', () => {
    ...
    it('should check if number is in range', () => {
      expect(numberUtils.inRange(3, 2, 4)).toBe(true);
      expect(numberUtils.inRange(2, 2, 4)).toBe(true);
      expect(numberUtils.inRange(4, 2, 4)).toBe(true);
      expect(numberUtils.inRange(1, 2, 4)).toBe(false);
      expect(numberUtils.inRange(5, 2, 4)).toBe(false);
    });

    // 应该处理 min > max 的情况
    it('should handle min > max', () => {
      expect(numberUtils.inRange(3, 4, 2)).toBe(true);
    });
  });
  ...
  describe('round', () => {
    ...
    it('should round to integer', () => {
      expect(numberUtils.round(4.006)).toBe(4);
      expect(numberUtils.round(4.5)).toBe(5);
    });

    // 应该四舍五入到指定精度
    it('should round to precision', () => {
      expect(numberUtils.round(4.006, 2)).toBe(4.01);
      expect(numberUtils.round(4.004, 2)).toBe(4);
    });

    // 应该处理负精度
    it('should handle negative precision', () => {
      expect(numberUtils.round(4060, -2)).toBe(4100);
    });
  });
});
